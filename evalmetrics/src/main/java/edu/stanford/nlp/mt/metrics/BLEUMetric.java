...
public class BLEUMetric<TK, FV> extends AbstractMetric<TK, FV> {
  ...
  protected final List<Map<Sequence<TK>, Integer>> maxReferenceCounts;
  protected final int[][] refLengths;
  protected final int order;
  protected final double multiplier;
  protected final boolean smooth;

  protected static final boolean printLocalScores = (System
  ...
  protected static int possibleMatchCounts(int order, int length) {
    int d = length - order;
    return d >= 0 ? d : 0;
  }

  protected static <TK> int[] localMatchCounts(Map<Sequence<TK>, Integer> clippedCounts, int order) {
  ...
  protected static int bestMatchLength(int refLengths[], int candidateLength) {
  ...
  public class BLEUIncrementalMetric implements
    ...
    protected BLEUIncrementalMetric() {
    ...
    protected BLEUIncrementalMetric(NBestListContainer<TK, FV> nbest) {
      r = 0;
      c = 0;
...
//    @Override
    ...
    protected double logScore(int max) {
      double ngramPrecisionScore = 0;

      double[] precisions = ngramPrecisions();
      double wt = 1.0 / max;
      for (int i = 0; i < max; i++) {
          if(!Double.isNaN(precisions[i])) {
